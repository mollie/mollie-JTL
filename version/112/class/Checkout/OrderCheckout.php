...
class OrderCheckout extends AbstractCheckout
{
        ...
        if ($this->getModel()->kID) {
            try {
                ...
                if (in_array($this->order->status, [OrderStatus::STATUS_COMPLETED, OrderStatus::STATUS_PAID, OrderStatus::STATUS_AUTHORIZED, OrderStatus::STATUS_PENDING], true)) {
                    // TODO TRANLATE
                    throw new RuntimeException("ERROR: ALREADY PAID!"); //self::Plugin()->getLocalization()->getTranslation('errAlreadyPaid'));
                }
    ...
    public function updateModel()
    {
        ...
        if ($this->payment) {
            ...
            $this->getModel()->cCheckoutURL = $this->payment->getCheckoutUrl();
        }
    ...
    public function loadRequest($options = [])
    {
        ...
        if (defined(get_class($pm) . '::METHOD') && $pm::METHOD !== ''
            && (self::Plugin()->oPluginEinstellungAssoc_arr['resetMethod'] !== 'on' || !$this->getMollie())) {
